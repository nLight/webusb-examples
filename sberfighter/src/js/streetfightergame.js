...
var game = new Phaser.Game(473, 240, Phaser.CANVAS, 'phaser-example', {
    preload: preload,
    create: create,
    update: update,
    render: render
...
function preload() {
    ...
    game.load.image('blankaLevel', 'src/sprites/factory.png');
...
function renderSprites() {
    ...
    enemyBison = game.add.sprite(300, YValueBlankaLevel, 'mbisonsprites');
    playerKen = game.add.sprite(100, YValueBlankaLevel, 'kennewsprites');
}

function playerKenSpritesLoad() {
    //Standing:
    playerKen.animations.add('standing', [0, 1, 2, 3]);
    playerKen.animations.add('walkingforward', [0]);
    playerKen.animations.add('walkingbackward', [0]);
...
function enemyBisonSpritesLoad() {
    //standing
    enemyBison.animations.add('standing', [0, 1, 2, 3]);
...
function inputDeclarations() {
    ...
    document.addEventListener("serial", (event) => {
      console.log("Serial event: ", event.detail);

      switch(event.detail) {
        case "A": simulateKeyPress(cursors.left); break;
        case "S": simulateKeyPress(cursors.right); break
        case "D": simulateKeyPress(lightPunchInput); break;
        case "F": simulateKeyPress(lightKickInput); break;
      }
